...
cat << "EOF"
	...
	NOT SUITABLE FOR VALIDATOR! YOU CAN USE blackfury-node.sh SCRIPT TO INSTALL FULL NODE WITH DISABLED RPC-API!
...
sleep 5s
ponysay "Installing blackfury from source code with updated genesis_29-2 mainnet!"
sleep 5s
ponysay "WARNING: cosmosvisor, evmosd processes will be killed and genesis, blackfury, evmos, evmosd system services will be stopped with this script on the next step. If you have other blockchains running, you might want to delete those parts of the script!"
sleep 20s
...
service blackfury stop
service evmos stop
service evmosd stop

# BACKUP PREVIOUS .blackfury
cd
rsync -r --verbose --exclude 'data' ./.blackfury/ ./.blackfury_backup/

# DELETING OF .blackfury FOLDER (PREVIOUS INSTALLATIONS)
cd 
rm -r .blackfury

# BUILDING blackfury BINARIES
cd blackfury
make install
export PATH=$PATH:$(go env GOPATH)/bin
echo 'export PATH=$PATH:$(go env GOPATH)/bin' >> ~/.bashrc
...
blackfury config chain-id genesis_29-2
blackfury config keyring-backend os
...
sleep 20s
blackfury keys add mygenesiskey --keyring-backend os --algo secp256k1 --account 0 --index 0 --coin-type 118
sleep 120s

blackfury init $1 --chain-id genesis_29-2 

#IMPORTING THE GENESIS STATE
cd 
cd .blackfury/config
rm -r genesis.json
wget https://github.com/FuryUnderverse/blackfury/raw/neolithic/genesis_29-1-state/genesis.json
cd

# RESET TO IMPORTED genesis.json
blackfury unsafe-reset-all
...
# STARTING blackfury AS A SERVICE
 cd
 cd /etc/systemd/system
 rm -r genesis.service
 wget https://raw.githubusercontent.com/FuryUnderverse/blackfury/neolithic/blackfuryrpc.service
 systemctl daemon-reload
 systemctl enable blackfuryrpc.service
 echo All set! 
 sleep 3s
...
sleep 5s
service blackfuryrpc start
# blackfuryrpc start
ponysay "blackfuryrpc node service with EVM RPC-API started, you may try *service blackfuryrpc status* command to see it! Welcome to the GenesisL1 blockchain! DON'T START VALIDATOR WITH RPC-API ENABLED!!!"
