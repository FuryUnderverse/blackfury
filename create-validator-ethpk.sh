...
cat << "EOF"
	...
	This script should create genesis_29-2 blockchain validator while running under root user,
  	...
  	Should be started after the full node sync, for example after blackfury-node.sh script run.
...
ponysay "This script will create your new GenesisL1 validator. You should run this script on fully synchronized node. For example after the blackfury-node.sh script run. You can check status of your node service with:
service blackfury status"
...
-COMMISSION_RATE(0.01 = 1%; 0.99 = 99%)
...
sleep 3s
./blackfury keys unsafe-import-eth-key importedethkey $2 --keyring-backend os
sleep 2s
ponysay "Submitting create validator transaction, enter your key password"
./blackfury tx staking create-validator \
  --amount=$3el1 \
  --pubkey=$(blackfury tendermint show-validator) \
  --moniker="$1" \
