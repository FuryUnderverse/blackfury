...
sleep 20s
blackfury keys add mygenesiskey --keyring-backend os --algo eth_secp256k1
sleep 120s
...
# STARTING blackfury AS A SERVICE
 ...
 wget https://raw.githubusercontent.com/FuryUnderverse/blackfury/neolithic/blackfury.service
 systemctl daemon-reload
 systemctl enable blackfury.service
 echo All set! 
 sleep 3s
...
sleep 5s
service blackfury start
# blackfury start
ponysay "blackfury node service started, you may try *service blackfury status* command to see it! Welcome to the GenesisL1 blockchain! After the full sync you can start validator! Full sync should take 30min to 6h depending on Internet Connectio!"
